...
class Analyzer:
    ...
    def _calc_quantile(self, p: float):
        if not 0.0 <= p <= 1.0:
    ...
    def _calc_mode(self):
        ...
        for k, v in self.columns.items():
            ...
            modes[k] = max(counts, key=counts.get)
        return modes
    ...
    def _calc_covariance(self, target: str):
    ...
    def covariance(self, target: str):
    ...
    def _calc_correlation(self, target: str):
        cov_res = self._calc_covariance(target)
        std = self._calc_std()
    ...
    def correlation(self, target: str):
